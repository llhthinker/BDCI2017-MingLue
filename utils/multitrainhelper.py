...
#    predicted = torch.sigmoid(outputs)
    ...
    predicted_max_labels = predicted.cpu().numpy().tolist()
    ...
    for pl_i in range(len(predicted_labels)):
        # print(predicted_max_labels[pl_i])
        predicted_labels[pl_i].append(predicted_max_labels[pl_i])
...
def do_eval(valid_loader, model, model_id, config, dmpv_model=None, dbow_model=None):
 ...
 #   loss_weight = get_loss_weight(predicted_labels, true_labels, 8)
    ...
    score = cs.jaccard(predicted_labels, true_labels)
    print("Jaccard:", score)
